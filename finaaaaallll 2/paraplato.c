#include "biblio.h"
//randomization du plateau
void creaPlato(char plato[21][27]) {
    // initialisation des 33 tuiles qui sont mobiles, avec leurs trésors respectifs
    char tuiletreL1[3][3] = {{'#', '_', '#'}, //initialisation des 6 tuiles en L avec trésor, avec différentes orientations
                             {'#', '!', '_'},
                             {'#', '#', '#'}};
    ...
    char tuiletreL6[3][3] = {{'#', '_', '#'},//...
                             {'_', '+', '#'},
                             {'#', '#', '#'}};

    char tuileL1[3][3] = {{'#', '_', '#'}, //initialisations des 10 tuiles en L sans trésors, avec différentes orientation
                          {'#', '_', '_'},
                          {'#', '#', '#'}};
    ...
    char tuileI1[3][3] = {{'#', '_', '#'},//initialisation des 12 tuiles en I sans trésors, avec différentes orientations
                          {'#', '_', '#'},
                          {'#', '_', '#'}};
    ...
    char tuileT1[3][3] = {{'#', '_', '#'},//initialisation des 6 tuiles en T, avec trésors, avec différentes orientations
                          {'#', '.', '_'},
                          {'#', '_', '#'}};
    ...
    // le code suivant initialise un tableau de 33 caractères(correspondant au 33 tuiles mobiles) et le mélange avec la boucle for.

    char tab[34] = {'a', 'z', 'e', 'r', 't', 'y', 'u', 'i', 'o',
                    'p', 'q', 's', 'd', 'f', 'g', 'h',
                    'j', 'k', 'l', 'm', '5', '6', '7',
                    'w', 'x', 'c', 'v', 'b', 'n', '1', '2', '3', '4','8'};
    ...
    // ici les différentes coordonnées de plato sont les 33 centres des 33 tuiles mobiles auxquelles on affecte un caractère provenant du tableau initialisé et mélangé plus tôt.
    // le srand(time(NULL)) garantit un mélange unique a chaque appel
    plato[1][5] = tab[0];
    plato[1][13] = tab[1];
    plato[1][21] = tab[2];
    ...
    //avec les 2 boucles for suivantes on parcourt plato, et en fonction du caractère trouvé au milieu d'une tuile, on la remplace par une autre
    for (int i = 0; i < 21; i++) {
        for (int j = 0; j < 27; j++) {
            if (plato[i][j] == 'a') {
...
void modiplato(char plato[21][27], char tuileEntrante[3][3], int *x, int *yp, int *x2, int *y2, int *x3, int *y3, int *x4, int *y4) {
    ...
    if (colonne == 0) {
        ligne = choixLigne();
        sens = choixGD();
        if (sens == 1) { // décalage en insérant la tuile par la gauche, donc vers la droite
        ...
        } else if (sens == 2) {      //décalage en insérant la tuile par la droite, donc vers la gauche
            if (ligne == 2) {
    ...
    } else {  //decalage des colonnes
        sens2 = choixHB();
        if (sens2 == 2) {  //décalage en insérant la tuile par la bas, donc vers la haut
            if (colonne == 2) {
        ...
        } else if (sens2 == 1) {     //décalage en insérant la tuile par le haut, donc vers le bas
            if (colonne == 2) {
    ...
    //changement des coordonnées de A B C et D s'ils sont poussés par un coulissement du plateau
    for (int i = 0; i < 21; i++) {
        for (int j = 0; j < 27; j++) {
            if (plato[i][j] == 'A') {
                *yp = i;
...
void tp(char tuile[3][3], int *x, int *y, int *x2, int *y2, int *x3, int *y3, int *x4, int *y4, int *erreur, int *erreur2,
    ...
    if (tuile[1][1] == 'A') {        //si sur la tuile qui vient de sortir se trouve le pion A
        changcoo(x, y);              //on met les coordonnées de A à jour
        tuile[1][1] = *tp;           //on récupère sur la tuile sortant ce sur quoi était le pion
        *tp = plato[*y][*x];         //mise à jour du caractère sur lequel se trouve A
        *erreur = 1;                 //fait en sorte que les coordonnées de A ne soient pas remises à jour
    }

    if (tuile[1][1] == 'B') {        //si sur la tuile qui vient de sortir se trouve le pion B
        changcoo(x2, y2);        //on met les coordonnées de B à jour
        tuile[1][1] = *tp2;         //on récupère sur la tuile sortant ce sur quoi était le pion
        *tp2 = plato[*y2][*x2];     //mise à jour du caractère sur lequel se trouve B
        *erreur2 = 1;               //fait en sorte que les coordonnées de A ne soient pas remises à jour
    }
    if (tuile[1][1] == 'C') {       //si sur la tuile qui vient de sortir se trouve le pion C
        changcoo(x3, y3);       //on met les coordonnées de C à jour
        tuile[1][1] = *tp3;         //on récupère sur la tuile sortant ce sur quoi était le pion
        *tp3 = plato[*y3][*x3];     //mise à jour du caractère sur lequel se trouve C
        *erreur3 = 1;               //fait en sorte que les coordonnées de A ne soient pas remises à jour
    }
    if (tuile[1][1] == 'D') {       //si sur la tuile qui vient de sortir se trouve le pion D
        changcoo(x4, y4);       //on met les coordonnées de D à jour
        tuile[1][1] = *tp4;         //on récupère sur la tuile sortant ce sur quoi était le pion
        *tp4 = plato[*y4][*x4];     //mise à jour du caractère sur lequel se trouve D
        *erreur4 = 1;               //fait en sorte que les coordonnées de A ne soient pas remises à jour
    }
};